...
double stayInRegion(double min, double max, double value);
...
void initializeParticlePositionAndVelocity_for2dim( void ){
  ...
  // 流体の移動を計算する場合、x, y方向のインクリメント幅を同じにしないと挙動が変になる
  for(iX= -4; iX<nX; iX++){ 
    for(iY= -4; iY<nY; iY++){
...
void moveParticle( void ){
  ...
  for(i=0;i<NumberOfParticles;i++){
    if(ParticleType[i] == FLUID){
      ...
      Position[i*3] = stayInRegion(0.0, 2.0, Position[i*3]);
      Position[i*3+1] = stayInRegion(0.0, 0.4, Position[i*3+1]);
  ...
  for (i=0; i<NumberOfParticles; i++){
    ...
    for (j=0; j<NumberOfParticles; j++){
      ...
      if ((sliderLeft<=Position[i*3])&&(Position[i*3]<=sliderRight)&&(0.4-PARTICLE_DISTANCE<=Position[i*3+1])&&(Position[i*3+1]<=0.4+PARTICLE_DISTANCE)){
        UpdatedTemp += df_coef*load*G_Y*stableSpeed*DT;
      }
    }
