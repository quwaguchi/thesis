...
void initializeParticlePositionAndVelocity_for2dim( void ){
  ...
  for(iX= 0; iX<nX; iX+=2){
    for(iY= 0;iY<nY;iY++){
      ...
      /* air region */
      if((x>=0.0 && x<1.5) && ((y>=0.015 && y<0.020) || (iX%100!=0 && y==0.014))){
        ParticleType[i] = AIR;
        Temperature[i] = sliderTemp;
        flagOfParticleGeneration = ON;
      } 
...
double sliderRight = -0.001;

void mainLoopOfSimulation( void ){
	FINISH_TIME = 2.0 + 2.0/sliderSpeed + 1.0;
  int iTimeStep = 0;
...
void calTemperature( void ){
  ...
  for (i=0; i<NumberOfParticles; i++){
    if (ParticleType[i] == SLIDER || ParticleType[i] == AIR){
      continue;
    }
    ...
    if ((ParticleType[i]==SOLID || ParticleType[i]==SOLID_AND_FLUID) && (Position[i*3]<=sliderRight) && (Position[i*3]>=sliderLeft) && (Position[i*3+1]>0.015-2*PARTICLE_DISTANCE)){
      UpdatedTemp += df_coef*1*9.8*sliderSpeed*DT/(rho*c*pow(PARTICLE_DISTANCE, 3)); 
    }  
