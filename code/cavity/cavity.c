...
void initializeParticlePositionAndVelocity_for2dim( void ){
  ...
  for(iX= -4;iX<nX;iX++){
    for(iY= -4;iY<nY;iY++){
      ...
      /* dummy wall region */
      if( ((x>-4.0*PARTICLE_DISTANCE+EPS)&&(x<=2.00+4.0*PARTICLE_DISTANCE+EPS))&&( (y>0.0-4.0*PARTICLE_DISTANCE+EPS )&&(y<=2.0+EPS)) ){  
	      ParticleType[i]=DUMMY_WALL;
	      flagOfParticleGeneration = ON;
      }

      /* wall region */
      if( ((x>-2.0*PARTICLE_DISTANCE+EPS)&&(x<=2.00+2.0*PARTICLE_DISTANCE+EPS))&&( (y>0.0-2.0*PARTICLE_DISTANCE+EPS )&&(y<=2.0+EPS)) ){ 
        ParticleType[i]=WALL;
        flagOfParticleGeneration = ON;
      }
      ...
      */
      ...
      /* fluid region */
      if( ((x>0.0+EPS)&&(x<=2.00+EPS)) &&((y>0.0+EPS)&&(y<=2.00+EPS)) ){ 
...
void moveParticle( void ){
  ...
  for(i=0;i<NumberOfParticles;i++){
    if(ParticleType[i] == FLUID){
      //追記
      if (2.00-PARTICLE_DISTANCE<=Position[i*3+1]){
         Velocity[i*3  ] = 0.1; 
         Velocity[i*3+1] = 0; 
         Velocity[i*3+2] = 0;
      } else {
      ...
      //追記
      Position[i*3+1] = (Position[i*3+1] <= 2.00) ? Position[i*3+1] : 2.00; 
    }
