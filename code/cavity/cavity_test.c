...
#define DIM                  2
#define PARTICLE_DISTANCE    0.05 //0.025
#define DT                   0.001
...
//追記
double stayInBox(double temp, double min, double max) {
  if (temp < min) {
    return min;
...
void initializeParticlePositionAndVelocity_for2dim( void ){
  ...
  nY = (int)(2.0/PARTICLE_DISTANCE)+5;
  for(iX= -4;iX<nX;iX++){
    for(iY= -4;iY<nY;iY++){
      ...
      /* dummy wall region */
      if( ((x>-4.0*PARTICLE_DISTANCE)&&(x<=2.0+4.0*PARTICLE_DISTANCE))&&( (y>0.0-4.0*PARTICLE_DISTANCE)&&(y<=2.0+4.0*PARTICLE_DISTANCE)) ){  
	      ParticleType[i]=DUMMY_WALL;
	      flagOfParticleGeneration = ON;
      }

      /* wall region */
      if( ((x>-2.0*PARTICLE_DISTANCE)&&(x<=2.0+2.0*PARTICLE_DISTANCE))&&( (y>0.0-2.0*PARTICLE_DISTANCE)&&(y<=2.0+2.0*PARTICLE_DISTANCE)) ){ 
        ParticleType[i]=WALL;
        flagOfParticleGeneration = ON;
      }
      ...
      /* empty region
      if ( ((x>0.50+EPS)&&(x<=1.00+EPS)) && ((y>0.0+EPS)&&(y<=0.60+EPS)) ){
         flagOfParticleGeneration = OFF;
      }
      */
...
void moveParticle( void ){
  ...
  for(i=0;i<NumberOfParticles;i++){
    if(ParticleType[i] == FLUID){
      //追記
      if ((2.0-PARTICLE_DISTANCE<=Position[i*3+1]) && (Position[i*3]<=2.0-PARTICLE_DISTANCE)){
         Velocity[i*3  ] = 10; 
         Velocity[i*3+1] = 0; 
         Velocity[i*3+2] = 0;
      } else {
      ...
      //追記
      Position[i*3  ] = stayInBox(Position[i*3  ]+Velocity[i*3  ]*DT, 0.0, 2.0);
      Position[i*3+1] = stayInBox(Position[i*3+1]+Velocity[i*3+1]*DT, 0.0, 2.0);
...
void moveParticleUsingPressureGradient( void ){
  ...
  for(i=0;i<NumberOfParticles;i++){
    if(ParticleType[i] == FLUID){
      ...
      //追記
      Position[i*3  ] = stayInBox(Position[i*3  ]+Acceleration[i*3  ]*DT*DT, 0.0, 2.0);
      Position[i*3+1] = stayInBox(Position[i*3+1]+Acceleration[i*3+1]*DT*DT, 0.0, 2.0);
