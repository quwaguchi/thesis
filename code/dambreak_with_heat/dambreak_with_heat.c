  ...
  for(iX= -4;iX<nX;iX++){
    for(iY= -4;iY<nY;iY++){
      ...
      if( ((x>0.0+EPS)&&(x<=0.25+EPS)) &&((y>0.0+EPS)&&(y<=0.50+EPS)) ){ 
        ...
        //追記
        if (((x>0.1+EPS)&&(x<=0.2+EPS)) && ((y>0.2+EPS)&&(y<=0.3+EPS))){
          Temperature[i] = 100;
        } 
...
void calTemperature( void ){
  ...
  a = 1.55e-4*(2.0*DIM)/(N0_forLaplacian*Lambda);
  for(i=0;i<NumberOfParticles;i++){
    if(ParticleType[i] != FLUID) continue;
    ...
    for(j=0;j<NumberOfParticles;j++){
      ...
      if(distance<Re_forLaplacian){
        w =  weight(distance, Re_forLaplacian);
        UpdatedTemp += a*(Temperature[j]-Temperature[i])*w*DT;
      }
    }
